...
If $X$ is reasonably large,
we can consider an SVD decomposition $X = V \Lambda V$

Additionally, the empirical covariance estimator performs poorly in high dimension.
Shrunk estimators like the one proposed by~\cite{ledoit_wolf} are proven to be superior in that situation.
Suppose $\cove = \frac{1}{n}X^\top X$ is the empirical estimator.
...
\end{equation*}
Finding the optimal coefficient $\delta$ can be done in $\cO\left( n^2 \cdot p \right)$ steps.

Randomized SVD algorithms~\citep{random_svd} may be employed to speed-up the process,
or to achieve alternating descent on $D$ and $U$.
We implement such algorithms and show that they scale moderately well with $n$ and $p$.
One single step is often enough.
